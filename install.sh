...
function setup_chatwoot() {
  ...
  git clone https://github.com/tbl07/wellcms.git
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  yarn
  ...
  rake assets:precompile RAILS_ENV=develop
EOF
}
...
function get_console() {
  sudo -i -u chatwoot bash -c " cd chatwoot && RAILS_ENV=develop bundle exec rails c"
}
...
function upgrade() {
  ...
  RAILS_ENV=develop bundle exec rake db:migrate
